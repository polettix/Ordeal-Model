...
get a deck, ordered. See also L</get_shuffled_cards>.

=head2 B<< get_shuffled_cards >>
...
This method uses L<Ordeal::Model::ShuffleSet::create> behind the scenes,
to which it forwards most of the parameters in C<%args>. Any parameter
described in L<Ordeal::Model::ShuffleSet> and not below is a
documentation bug!
...
L<Ordeal::Model::ShuffleSet>. The exact shape of
an input item is described in L<Ordeal::Model::ShuffleSet>; in this
...
call L<Ordeal::Model::ShuffleSet::draw> this number of times. Defaults
to C<1>.
...
the random data source. It is passed to method C<restore> of whatever
random data source is available, i.e. either the one provided by
C<%args> or, as a fallback, an instance of L<Ordeal::Model::ChaCha20>.

=item C<< seed >>

this parameter is consumed internally in this method, and in particular
it is used for creating a new instance of L<Ordeal::Model::ChaCha20> if
L</random_source> is not present.
...
C<$args{n}> draws sequentially, like this:
...
Note that the C<$iterator> function always returns a list; hence, you
