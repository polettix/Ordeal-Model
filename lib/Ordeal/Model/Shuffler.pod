...
this:
   ...
   # also present with alternative syntax, again same as French@[0..8]
   French@[#9]

Note the difference: C<"deck"9> takes the first 9 cards from C<deck>,
while C<deck[9]> takes the 10th card only. The double quotes in the
first case are necessary to separate the deck name from the number of
cards to deal.
