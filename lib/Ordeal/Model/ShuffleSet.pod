...
=head2 B<< auto_reshuffle >>
   ...
   my $boolean = $shfls->auto_reshuffle;
   $shfls->auto_reshuffle($boolean);

accessor for automatically calling L</reshuffle> after every invocation
L</draw>.
...
random source set in a definition overrides this. If not set, each shuffle
will have its own random source.
