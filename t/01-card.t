...
throws_ok { $model->get_card('inexistent-1-ciao.gif') } qr{invalid extension},
   'invalid extension';
...
is $card->name, 'wtf', 'card name';
is $card->group, 'group1', 'card group';
...
is $_->group, 'group1', 'card is in right group'
   for @group1;
...
my @mixed = $model->get_cards(query => {group => [qw< group2 public >]});
is scalar(@mixed), 3, '3 items from mixed query';

@mixed = $model->get_cards(query =>
   {group => 'group2', id => 'group1-03-wtf.svg'});
